...
| F18 | **Reaction refresh via widened polling**: reactions land on already-fetched messages, which the watermark fetch (F8) never re-ships — so per conversation, at most once per 15 minutes (`REACTION_REFRESH_INTERVAL`), a poll visit widens its `oldest` to the *older of* the conversation's watermark and a 24-hour window (`REACTION_WINDOW_SECS` — never narrower than the watermark, or a quiet conversation's unseen messages would be skipped). Re-shipped messages merge through the history-authoritative upsert: stored reactions are replaced whenever they differ, including replacement by an empty set (a removal is real on this path). The *live* socket path is non-authoritative for reactions: an event without them inherits what is stored, never clears it. Widened-fetch pages are charged to the same request budget as everything else. |
