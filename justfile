# herdr-slackr dev tasks — run `just <task>` (https://github.com/casey/just)
...
# run the pane in the current repo
run:
    cargo run
...
install:
    ...
    rm -f bin/herdr-slackr
    cp target/release/herdr-slackr bin/herdr-slackr
    [ "$(uname)" = "Darwin" ] && codesign --force --sign - bin/herdr-slackr || true
