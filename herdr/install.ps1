# herdr `[[build]]` step, Windows twin of install.sh: download the prebuilt herdr-slackr
...
# $HERDR_PLUGIN_ROOT\bin\herdr-slackr.exe.
...
# Post-install next steps: printed on success only, never affects exit status. Points at the
# agent skill install (the CLAUDE.md reminder it prints matters more than the pane itself for
# agents that never open a herdr session interactively) and where tokens/config live.
Write-Output "${Name}: next steps"
Write-Output "  1) install the agent skill:  npx skills add dcieslak19973/herdr-slackr --skill herdr-slackr -g"
Write-Output "     (or: $Name skill-install . or: herdr plugin action invoke skill-install-windows --plugin dcieslak19973.slackr)"
Write-Output "  2) tokens:  `$env:HERDR_PLUGIN_CONFIG_DIR\tokens.toml or SLACK_APP_TOKEN/SLACK_USER_TOKEN"
Write-Output "     config:  `$env:HERDR_PLUGIN_CONFIG_DIR\config.toml (channels = [...])"
